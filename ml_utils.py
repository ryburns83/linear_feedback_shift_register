...
# Numpy
from numpy import zeros, roll, array, vstack, arange, dstack, expand_dims
...
def labeled_state_transition_obs(LFSRs, coeff_catalog, enable_tapped=False):
    """
    DESCRIPTION:

    INPUTS & OUTPUTS:
    :param LFSRs: list of LFSR() of some common fixed degree/order
    :type LFSRs: list, dtype=galois_tools.LFSR
    :param coeff_catlog: list of hex primitive polynomial coefficients
    :type coeff_catlog: list, dtype=str
    :param enable_tapped: build state vector by tapping LFSR w/ 0x1
    :type enable_tapped: bool
    :returns: LFSR state observations (n, n + 1) & polynomial labels
    :rtype: nunmpy.ndarray (x2 outputs)
    """
    # Order of LFSRs (assumes all same)
    deg = LFSRs[0].N

    # Max-length period of LFSRs
    M = 2**deg - 1

    # Number of polynomials = number of LFSRs
    N_polynomials = len(LFSRs)

    #######################################################
    # Observation vectors by streaming LFSR tapped at 0x1 #
    #######################################################

    # Initialize LFSR state/observations storage
    X = zeros([M * N_polynomials, deg, 2])

    # Initialize LFSR polynomial label storage
    Y = zeros([M * N_polynomials, deg, 1])

    # Tap LFSR via "& 0x1"?...
    if enable_tapped:

        # For each p'th primitive polynomial...
        for p in range(len(coeff_catalog)):

            # Current m-sequence indices
            idx = arange(p * M,(p + 1) * M)

            # Observations formed by tapping a p'th LFSR from a list of options
            x_p, y_p = form_tap_based_obs(LFSRs, p, deg)

            # Stack LFSR state observations (n, n + 1)
            X[idx, :, :] = dstack((x_p, y_p))

            # Use primitive polynomial underlying recursion @ n
            # to define a binary length-deg LFSR observation label
            Y[idx, :, :] = expand_dims(str2vec(int2bin(
                LFSRs[p].feedback_polynomial, deg)), axis=1)
    ...
    # Use state sequence...
        ...
        for p in range(len(coeff_catalog)):
            ...
            x_p, y_p = register_state_observations(LFSRs[p])

            # Stack LFSR state observations (n, n + 1)
            X[p * M:(p + 1) * M, :, :] = dstack((x_p, y_p))
